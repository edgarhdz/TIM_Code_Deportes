CQ.form.YoutubeVideoGallerySearch = CQ.Ext.extend(CQ.Ext.form.ComboBox, {
...
constructor : function(config) {
        ...
        var storeConfig = CQ.Util.applyDefaults(config.storeConfig, {
            "proxy":new CQ.Ext.data.HttpProxy( {
                "url" :"/bin/generics/articlesearch.video.json",
                "method" :"GET"
            }),
            "baseParams": {
                "videoType":"13"
    ...
    validator : function(){
		var isValid;
        var validation = searchFieldValidation("/apps/deportes/local/templates/video/video", this.getValue());
        if(!validation){
			isValid = CQ.I18n.getMessage("commons.searchfieldvalidation.youtube");
        }else{
			var correctVideoType = videoValidation(13, this.getValue());
            if(correctVideoType){
				isValid = true;
            }else{
				isValid = CQ.I18n.getMessage("commons.searchfieldvalidation.youtube");
...
CQ.Ext.reg("youtubevideogallerysearch", CQ.form.YoutubeVideoGallerySearch); //Register the xtype
...
MN_geo={ip:"190.242.97.218", allowed:true, city:"guatemala city", state:"gu", country:"gtm", dma:"-1"}
    ...
    jsCallbackHandler: function(objId, eventName, data)
    {
        switch (eventName)
        {
            case "jsApiReady":
                //	loadFeed();
                playerReady = 1;
                //console.log("Player listo " + playerReady);
                break;
...
function cambiaVideo(urlvideo){
    ...
    if( playerReady == 1 && adPreFinish == 1 && (changedSign != urlvideoTmp || changedSign == "")) {

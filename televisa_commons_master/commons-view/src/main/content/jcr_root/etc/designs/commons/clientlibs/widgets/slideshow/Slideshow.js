...
CQ.form.Slideshow = CQ.Ext.extend(CQ.form.SmartImage, {
    ...
    /**
     * @cfg {Number} maxItems Maximum number of items that can be created (defaults to 3).     
     */
    maxItems: 3,
    noItmes: 0,
    ...
    createSlide: function(data, slideIndex, basePath) {
        ...
        var headline = (data ? data["alt"] : null);
       // var description = (data ? data["description"] : null);
        var link = (data ? data["link"] : null);
...
//    },
    ...
    saveChanges: function() {
        ...
        if (this.editedSlide) {
            ...
            var headline = this.footPanel.getHeadline();
           // var description = this.footPanel.getDescription();
            var link = this.footPanel.getLink();
            ...
            this.editedSlide.headline = (headline ? headline : null);
            //this.editedSlide.description = (description ? description : null);
            this.editedSlide.link = (link ? link : null);
        }
    },
    ...
    showSlide: function(slide) {
        ...
        this.footPanel.setHeadline(slide.headline);
        //this.footPanel.setDescription(slide.description);
        this.footPanel.setLink(slide.link);
    ...
    onAddButton: function() {
        if(this.slides.length - this.noItmes <=  this.maxItems -1 ){
            this.addNewSlide();
        	this.buildComboBoxContent();
        }else{
			var messageMaxNumbersAwards = 'Permitted number of awards '+this.maxItems;
            CQ.Ext.Msg.alert('Status', messageMaxNumbersAwards);
        }
    },
...
// register xtype
CQ.Ext.reg('slideshowstantec', CQ.form.Slideshow);
